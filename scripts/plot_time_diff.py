...
df["edit_distance_diff"] = (df["time_taken_t1"] - df["time_taken_t2"]).abs()
...
sns.heatmap(pivot, annot=True, fmt=".0f", cmap="Reds", cbar_kws={"label": "Diferença |t1 - t2|"})
plt.title(f"Diferença absoluta da distância de edição ({tree_type} Tree)")
plt.xlabel("Tamanho de T2")
plt.ylabel("Tamanho de T1")
plt.tight_layout()
plt.savefig(f"{plots_path}/heatmap_diff_time_taken_{tree_type}_tree.png")
# plt.show()
