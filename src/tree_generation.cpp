...
std::string to_string(TreeType type) {
    switch (type) {
        case TreeType::Binary:   return "Binary";
        case TreeType::Linear: return "Linear";
        case TreeType::Star: return "Star";
        case TreeType::Random: return "Random";
        case TreeType::Shallow: return "Shallow";
...
Graph generate_tree(size_t n, TreeType type, Graph::DataStructures ds = Graph::AdjacencyList) {
    ...
    switch (type) {
		...
		// FIX: Should be while (tree.vertices() < n - 1)
